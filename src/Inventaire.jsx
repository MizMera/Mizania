...
function Inventaire() {
  ...
  useEffect(() => { getInventaire(); }, []);

  const handleProduitAjoute = () => { getInventaire(); };
  ...
  return (
    <Box sx={{ height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Box sx={{ flexShrink: 0 }}>
        <Typography variant="h4" sx={{ fontWeight: 800, mb: 2 }}>Gestion de l'Inventaire</Typography>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField fullWidth variant="outlined" placeholder="Rechercher par nom ou SKU..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} size="small" />
          <Button variant="outlined" startIcon={<PictureAsPdf />} onClick={handleExportPDF} sx={{ minWidth: 160 }}>Export PDF</Button>
        </Box>
      </Box>
      <Box sx={{ flex: 1, display: 'flex', gap: 2, minHeight: 0, flexDirection: { xs: 'column', lg: 'row' } }}>
        <Box sx={{ width: { xs: '100%', lg: '300px' }, flexShrink: 0 }}>
          <Paper sx={{ p: 2, height: '100%', overflow: 'auto' }}>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>Ajouter un Produit</Typography>
            <AjouterProduitForm onProduitAjoute={handleProduitAjoute} />
          </Paper>
        </Box>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Paper sx={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 8px 24px rgba(15,23,42,0.06)' }}>
            <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider', flexShrink: 0 }}>
              <Typography variant="h6" sx={{ fontWeight: 600 }}>Inventaire ({filteredInventaire.length} produits)</Typography>
            </Box>
            {chargement ? (
              <Box sx={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Typography>Chargement...</Typography></Box>
            ) : (
                      ...
                      {filteredInventaire.map((produit) => {
                        ...
                        return (
                          <TableRow key={produit.id} hover>
                            <TableCell>
                              {isEditing ? (
                                <TextField size="small" value={editValues.nom || ''} onChange={(e) => setEditValues({ ...editValues, nom: e.target.value })} sx={{ minWidth: 120 }} />
                              ) : produit.nom}
                            </TableCell>
                            <TableCell sx={{ display: { xs: 'none', sm: 'table-cell' } }}>
                              {isEditing ? (
                                <TextField size="small" value={editValues.sku || ''} onChange={(e) => setEditValues({ ...editValues, sku: e.target.value })} sx={{ minWidth: 100 }} />
                              ) : (produit.sku || '—')}
                            </TableCell>
                            <TableCell>
                              {isEditing ? (
                                <TextField size="small" type="number" value={editValues.quantite_stock || ''} onChange={(e) => setEditValues({ ...editValues, quantite_stock: e.target.value })} sx={{ minWidth: 80 }} />
                              ) : (
                                <Box sx={{ color: produit.quantite_stock < 5 ? '#EF4444' : 'inherit', fontWeight: produit.quantite_stock < 5 ? 'bold' : 'normal' }}>{produit.quantite_stock}</Box>
                              )}
                            </TableCell>
                            <TableCell align="right" sx={{ display: { xs: 'none', sm: 'table-cell' } }}>
                              {isEditing ? (
                                <TextField size="small" type="number" value={editValues.prix_achat || ''} onChange={(e) => setEditValues({ ...editValues, prix_achat: e.target.value })} sx={{ minWidth: 100 }} />
                              ) : (`${pa.toFixed(2)} DT`)}
                            </TableCell>
                            <TableCell align="right">
                              {isEditing ? (
                                <TextField size="small" type="number" value={editValues.prix_vente || ''} onChange={(e) => setEditValues({ ...editValues, prix_vente: e.target.value })} sx={{ minWidth: 100 }} />
                              ) : (`${pv.toFixed(2)} DT`)}
                            </TableCell>
                            <TableCell align="right" sx={{ display: { xs: 'none', sm: 'table-cell' }, color: marge > 0 ? '#22C55E' : '#EF4444', fontWeight: 'bold' }}>{marge.toFixed(2)} DT</TableCell>
                            <TableCell sx={{ display: { xs: 'none', sm: 'table-cell' } }}>
                              {isEditing ? (
                                <TextField size="small" select SelectProps={{ native: true }} value={editValues.type_article || ''} onChange={(e) => setEditValues({ ...editValues, type_article: e.target.value })} sx={{ minWidth: 140 }}>
                            ...
                            <TableCell>
                              {isEditing ? (
                                <Box sx={{ display: 'flex', gap: 0.5 }}>
                                  <IconButton size="small" color="primary" onClick={() => handleSave(produit.id)}><Save /></IconButton>
                                  <IconButton size="small" color="secondary" onClick={handleCancel}><Cancel /></IconButton>
                                </Box>
                              ) : (
                                <Box sx={{ display: 'flex', gap: 0.5 }}>
                                  <IconButton size="small" color="primary" onClick={() => handleEdit(produit)}><Edit /></IconButton>
                                  <IconButton size="small" color="error" onClick={() => handleDelete(produit.id)}><Delete /></IconButton>
                                </Box>
                              )}
                            </TableCell>
                          </TableRow>
